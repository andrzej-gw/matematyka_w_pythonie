...
def przeciecie(a,b):
    if isinstance(a,Prosta) and isinstance(b,Prosta):
        ...
        if b.a==696969:
            ...
            b=tmp
        if a.a==696969:
            return Zespolona(a.b,a.b*b.a+b.b)
    ...
    if isinstance(a,Okrag) and isinstance(b,Okrag):
        ...
        b=o2.r
        c=abs(o1.O-o2.O)
        alpha=rad_to_zesp(acos((a*a+c*c-b*b)/(2*a*c)))
        P1=(o2.O-o1.O)*alpha/abs(o2.O-o1.O)*o1.r+o1.O
        P2=odbicie(P1,Prosta(o1.O,o2.O))
        return [P1,P2]
...
def jednokladnosc(a,O,k):
	if isinstance(a,Zespolona):
		return (a-O)*k+O
	if isinstance(a,Prosta):
		r=rzut(O,a)
		if(abs(O-r)<precision):
			return a
		return prosta_rownolegla(jednokladnosc(r,O,k),a)
	if isinstance(a,Okrag):
		return Okrag((a.O-O)*k+O,a.r*k)
	print("Tego jeszcze nie umiem.")
	a=5/0
def styczne(A,o):
	if abs(o.O-A)<o.r:
		print("Punkt jest w srodku okregu.")
		a=5/0
	p=przeciecie(o,Okrag(A,sqrt(abs(o.O-A)**2-o.r**2)))
	res=[]
	for i in p:
		res.append(Prosta(i,A))
	return res
def inwersja(a,o):
	if isinstance(a,Zespolona):
		if(a==o.O):
			return Zespolona(696969,696969)
		if(a==Zespolona(696969,696969)):
			return o
		return (a-o.O)*o.r*o.r/abs(a-o.O)/abs(a-o.O)
	if isinstance(a,Prosta):
		if(abs(o.O-rzut(o.O,a))<precision):
			return a
		P=inwersja(rzut(o.O,a),o)
		return Okrag((o.O+P)/2,abs((o.O-P)/2))
	if isinstance(a,Okrag):
		if o.O==a.O:
			return Okrag(o.O,o.r*o.r/a.r)
		if(abs(abs(o.O-a.O)-a.r)<precision):
			b=inwersja(2*a.o-o.o,o)
			return prosta_prostopadla(b,Prosta(b,o.O))
		p=przeciecie(a,Prosta(o.O,a.O))
		p[0]=inwersja(p[0],o)
		p[1]=inwersja(p[1],o)
		return Okrag((p[0]+p[1])/2,abs(p[0]-p[1])/2)
	print("Tego jeszcze nie umiem.")
	a=5/0
# tracer(0)
# rysuj_siatke()
